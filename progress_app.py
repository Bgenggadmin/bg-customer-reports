...
try:
    ...
    st.error("Missing 'fpdf2' in requirements.txt. Please add it and redeploy.")
    st.stop()
...
class BG_Report(FPDF):
    def header(self):
        if os.path.exists("logo.png"):
            # Adjust x, y, w as needed
            self.image("logo.png", 10, 8, 33)
        
        self.set_font("Helvetica", "B", 16)
        self.cell(80) 
        self.cell(110, 10, "B&G ENGINEERING INDUSTRIES", new_x="LMARGIN", new_y="NEXT", align='R')
        self.set_font("Helvetica", "I", 10)
        self.cell(0, 5, "PROJECT PROGRESS REPORT", new_x="LMARGIN", new_y="NEXT", align='R')
        self.ln(15)
...
with st.form("main_form"):
    ...
    with c2:
        job_code = st.text_input("B&G Job Code", placeholder="e.g. BG-123")
    ...
    for m in milestones:
        ...
        with col_s:
            s = st.selectbox(f"{m}", ["In-Progress", "Completed", "Pending", "N/A"], key=f"s_{m}")
        with col_r:
            r = st.text_input(f"Remarks for {m}", key=f"r_{m}")
        ms_results[m] = {"status": s, "remark": r}

    st.divider()
    st.subheader("📸 Progress Photos")
    cam_photo = st.camera_input("📷 Take Photo (Mobile)")
    uploaded_files = st.file_uploader("📁 Or Upload Gallery", accept_multiple_files=True)
    ...
    if not customer or not equipment_name:
        ...
        try:
            ...
            def draw_row(label1, val1, label2, val2):
                pdf.cell(45, 10, label1, 1, 0, 'L', True)
                pdf.cell(50, 10, str(val1), 1, 0)
                pdf.cell(45, 10, label2, 1, 0, 'L', True)
                pdf.cell(50, 10, str(val2), 1, 1)
