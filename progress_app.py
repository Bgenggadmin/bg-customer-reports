...
def create_report_pdf(logs_list):
    ...
    for log in logs_list:
        ...
        pdf.cell(0, 8, f" JOB CODE: {log.get('job_code')} | DATE: {now_ist}", 1, 1, "C", fill=True)
        ...
        pdf.ln(3); pdf.cell(60, 7, " Milestone", 1, 0, 'L', True); pdf.cell(35, 7, " Status", 1, 0, 'L', True); pdf.cell(95, 7, " Remarks", 1, 1, 'L', True)
        ms = [
...
with t1:
    with st.form("entry_form", clear_on_submit=True):
        ...
        po_n, po_d, eng = c4.text_input("PO No."), c5.date_input("PO Date", value=datetime.now(IST)), c6.text_input("Engineer")
        c7, c8 = st.columns(2)
        ...
        s3, n3 = m_row("RM Status", "s3", "n3"); s4, n4 = m_row("Sub-deliveries", "s4", "n4")
        s5, n5 = m_row("Fabrication Status", "s5", "n5"); s6, n6 = m_row("Buffing Status", "s6", "n6")
        s7, n7 = m_row("Testing Status", "s7", "n7"); s8, n8 = m_row("QC Status", "s8", "n8")
        s9, n9 = m_row("FAT Status", "s9", "n9")

        up_photos = st.file_uploader("Upload Photos", accept_multiple_files=True)
    ...
    if data:
        ...
        for log in data:
            ...
            if c_d.button("🗑️", key=f"del_{log['id']}"):
                try:
                    f_list = conn.client.storage.from_("progress-photos").list(path=str(log['id']))
                    if f_list: conn.client.storage.from_("progress-photos").remove([f"{log['id']}/{f['name']}" for f in f_list])
                except: pass
                conn.table("progress_logs").delete().eq("id", log['id']).execute(); st.rerun()
            
            with st.expander("Show Photos & Details"):
                # DEBUG & PHOTO FETCH
                target_id = str(log['id'])
                plist = conn.client.storage.from_("progress-photos").list(path=target_id)
                
                if plist:
                    pcols = st.columns(5)
                    for i, p in enumerate(plist):
                        u = conn.client.storage.from_("progress-photos").get_public_url(f"{target_id}/{p['name']}")
                        pcols[i % 5].image(u, use_container_width=True)
                else:
                    st.error(f"DEBUG: No photos found in folder: '{target_id}'")
